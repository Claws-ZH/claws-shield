...
export function searchAll(db: IntelDatabase, query: string): SearchResult[] {
  ...
  for (const flag of db.featureFlags) {
    if (flag.flag.toLowerCase().includes(q) || flag.decodedPurpose.toLowerCase().includes(q) || flag.description.toLowerCase().includes(q)) {
  ...
  for (const cn of db.codenames) {
    if (cn.codename.toLowerCase().includes(q) || (cn.mapsTo && cn.mapsTo.toLowerCase().includes(q)) || cn.usage.toLowerCase().includes(q)) {
      results.push({ type: "codename", item: cn, matchField: "codename", matchValue: cn.codename })
    }
  }

  for (const tool of db.unreleasedTools) {
    if (tool.name.toLowerCase().includes(q) || tool.description.toLowerCase().includes(q) || tool.capabilityArea.toLowerCase().includes(q)) {
      results.push({ type: "tool", item: tool, matchField: "name", matchValue: tool.name })
    }
  }

  for (const ep of db.telemetryEndpoints) {
    if (ep.url.toLowerCase().includes(q) || ep.host.toLowerCase().includes(q) || ep.classification.toLowerCase().includes(q)) {
