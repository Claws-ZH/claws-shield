...
export function queryFlags(db: IntelDatabase, opts: FlagQueryOptions) {
	...
	if (opts.name) {
		...
		results = results.filter(
			(f: any) => f.flag.toLowerCase().includes(q) || f.decodedPurpose.toLowerCase().includes(q),
		)
	}
	if (opts.prefix) {
...
export function queryKillswitches(
	db: IntelDatabase,
	opts?: { remoteControlled?: boolean; riskLevel?: string },
) {
