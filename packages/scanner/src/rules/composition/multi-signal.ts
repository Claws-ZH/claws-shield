...
export const multiSignalRule: ScanRule = {
  ...
  async run(context: ScanContext): Promise<ScanFinding[]> {
    ...
    const hasOutbound = /fetch\s*\(|axios\.|curl\s+|wget\s+|https?:\/\/(?!localhost)/i.test(allContent)
    ...
    if (hasEnvAccess && hasOutbound) {
      findings.push({
        ...
        description: "Skill accesses environment variables AND makes outbound network calls. High risk of credential exfiltration.",
        confidence: 0.85,
    ...
    if (hasBase64 && hasShellExec) {
      findings.push({
        ...
        description: "Skill decodes base64 content AND executes shell commands. Classic malware pattern.",
        confidence: 0.9,
        recommendation: "Do not install. Decode all base64 strings and inspect what is being executed.",
      })
    }
