...
const EXFIL_PATTERNS = [
  { pattern: /process\.env(?!\[['"](?:NODE_ENV|PATH|HOME|SHELL|TERM|USER|LANG)\b)/g, desc: "process.env access" },
  { pattern: /\$\{?(?:API_KEY|SECRET|TOKEN|PASSWORD|CREDENTIAL|AUTH)/gi, desc: "secret variable reference" },
      ...
      for (const { pattern, desc } of EXFIL_PATTERNS) {
        ...
        if (match) {
          ...
          findings.push({
            ...
            confidence: 0.7,
            recommendation: "Verify this env access is necessary and data isn't being sent externally.",
          })
        }
      }
    }
