...
import { getModelCost, calculateCost, getQualityScore } from "../src/router/cost-model.js"
...
describe("Router", () => {
  ...
  it("throws when all providers in chain fail", async () => {
    ...
    await expect(
      router.route({ ...baseReq, model: "fail-model" }),
  ...
  it("routes by cheapest strategy", async () => {
    ...
    const expensiveProvider = createMockProvider("anthropic", ["claude-opus-4-6"], true, async (req) => ({
      id: "resp-expensive",
  ...
  it("routes by best_quality strategy", async () => {
    ...
    const highQProvider = createMockProvider("anthropic", ["claude-opus-4-6"], true, async (req) => ({
      id: "resp-high",
      model: req.model ?? "claude-opus-4-6",
      provider: "anthropic",
  ...
  it("skips unhealthy providers", async () => {
    ...
    const healthyProvider = createMockProvider("gemini", ["gemini-2.0-flash"], true, async (req) => ({
      id: "resp-healthy",
      model: req.model ?? "gemini-2.0-flash",
      provider: "gemini",
