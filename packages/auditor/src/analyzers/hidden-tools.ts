  ...
  async analyze(ctx: AuditContext, target: TargetInfo): Promise<void> {
    ...
    if (kbMatchedFlags.length > 0) {
      ...
      ctx.addFinding({
        ...
        description: `Found ${kbMatchedFlags.length} feature flag(s) matching the knowledge base: ${kbMatchedFlags.slice(0, 5).map((h) => h.match).join(", ")}`,
        evidenceIds,
    ...
    if (kbMatchedTools.length > 0) {
      ...
      ctx.addFinding({
        ...
        description: `Found ${kbMatchedTools.length} reference(s) to known unreleased tools: ${kbMatchedTools.slice(0, 5).map((h) => h.match).join(", ")}`,
        evidenceIds,
        recommendation: "Clearly mark unreleased features and obtain user consent before enabling them.",
      })
    }
    ...
    if (unreleasedHits.length > 0) {
      ctx.addFinding({
        ...
        evidenceIds: ctx.getEvidenceByType("feature_flag").slice(0, 5).map((e) => e.id),
      })
    }
  }
}
