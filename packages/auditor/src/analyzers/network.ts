  ...
  async analyze(ctx: AuditContext, target: TargetInfo): Promise<void> {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const pattern of HIGH_FREQUENCY_PATTERNS) {
          ...
          while ((fm = pattern.exec(line)) !== null) {
            const interval = parseInt(fm[1], 10)
    ...
    if (thirdPartyDomains.size > 5) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Document all third-party connections. Minimize external dependencies and data sharing.",
      })
    }
    ...
    if (exfilHits.length > 0) {
      ctx.addFinding({
        ...
        evidenceIds: ctx.evidence
          ...
          .slice(0, 5)
          .map((e) => e.id),
        recommendation: "Review all outbound POST requests. Never transmit credentials, tokens, or sensitive environment data.",
      })
    }
    ...
    if (frequencyHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds: ctx.evidence
          ...
          .slice(0, 3)
          .map((e) => e.id),
        recommendation: "Use reasonable polling intervals. Consider server-sent events or webhooks instead of polling.",
      })
    }
