import { AUDIT_CATEGORY_WEIGHTS, generateId } from "@claws-shield/core"
...
export class PrivacyAnalyzer implements Analyzer {
	...
	async analyze(ctx: AuditContext, target: TargetInfo): Promise<void> {
		...
		const weightedCategories = (
			Object.entries(AUDIT_CATEGORY_WEIGHTS) as [FindingCategory, number][]
		)
			.filter(([_, w]) => w > 0)
		...
		if (
			...
			ctx.addFinding({
				...
				description:
					"Combination of undeactivatable telemetry and device fingerprinting creates a persistent tracking mechanism that users cannot opt out of.",
				evidenceIds: [
					...ctx.getEvidenceByType("endpoint").slice(0, 3),
					...ctx.getEvidenceByType("env_fingerprint").slice(0, 3),
				].map((e) => e.id),
				recommendation:
					"Implement genuine opt-out for all telemetry. Remove device fingerprinting or make it opt-in.",
			})
		}
		...
		if (
			...
			ctx.addFinding({
				...
				description:
					"Undercover capabilities combined with potential data exfiltration patterns suggest data may be collected while obscuring AI involvement.",
				evidenceIds: [
					...ctx.getEvidenceByType("attribution_rule").slice(0, 2),
					...ctx.getEvidenceByType("endpoint").slice(0, 2),
				].map((e) => e.id),
				recommendation:
					"Ensure complete transparency about data collection. Separate AI attribution from data handling.",
			})
		}
	}
}
...
function buildPrivacySummary(
	...
	parts.push(
		`Total findings: ${totalFindings} (${severityCounts.critical} critical, ${severityCounts.high} high, ${severityCounts.medium} medium, ${severityCounts.low} low, ${severityCounts.info} info).`,
	)

	if (worstCategories.length > 0) {
		parts.push(
			`Areas of concern: ${worstCategories.map(([cat]) => CATEGORY_LABELS[cat]).join(", ")}.`,
		)
	}
...
function buildRecommendations(
	...
	for (const [cat] of worstCategories) {
		switch (cat) {
			...
			case "network":
				recs.push(
					"Audit all third-party connections. Prevent credential and environment variable leakage.",
				)
				break
			case "undercover":
