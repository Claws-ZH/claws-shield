  ...
  async analyze(ctx: AuditContext, target: TargetInfo): Promise<void> {
    ...
    if (settingsHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Make remote configuration changes transparent and require user opt-in. Provide local overrides.",
      })
    }

    if (pollingHits.length > 0) {
      const evidenceIds = ctx.getEvidenceByType("polling_pattern")
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Document polling behavior and allow users to control polling frequency or disable it.",
      })
    }

    if (forceUpdateHits.length > 0) {
      const evidenceIds = ctx.getEvidenceByType("killswitch")
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Allow users to defer updates. Never block tool usage based on version checks.",
    ...
    if (modelOverrideHits.length > 0) {
      ctx.addFinding({
        ...
        evidenceIds: ctx.getEvidenceByType("polling_pattern").slice(0, 3).map((e) => e.id),
        recommendation: "Always respect user model selection. Log any server-side model substitutions visibly.",
      })
    }
    ...
    if (killswitchHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Document all remote kill capabilities. Provide graceful degradation rather than hard kills.",
      })
    }
    ...
    if (kbKillswitches.length > 0 && killswitchHits.length > 0) {
      ctx.addFinding({
        ...
        evidenceIds: ctx.getEvidenceByType("killswitch").slice(0, 3).map((e) => e.id),
      })
    }
  }
}
