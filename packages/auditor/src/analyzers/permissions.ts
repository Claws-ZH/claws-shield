  ...
  async analyze(ctx: AuditContext, target: TargetInfo): Promise<void> {
    ...
    if (shellHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Implement allowlisting for shell commands. Require explicit user approval before execution.",
      })
    }
    ...
    if (fsHits.length > 10) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Restrict filesystem access to the project directory. Implement a permission sandbox.",
      })
    }
    ...
    if (sensitivePathHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Never access sensitive paths without explicit user consent. Block access to credential files by default.",
      })
    }
    ...
    if (elevatedHits.length > 0) {
      ctx.addFinding({
        ...
        evidenceIds: ctx.getEvidenceByType("permission_request").slice(0, 5).map((e) => e.id),
        recommendation: "AI tools should never require or use elevated privileges. Follow principle of least privilege.",
      })
    }
  }
}
