  ...
  async analyze(ctx: AuditContext, target: TargetInfo): Promise<void> {
    ...
    if (attributionHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Always maintain transparent AI attribution. Allow users to choose disclosure level.",
      })
    }
    ...
    if (impersonationHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Remove instructions that direct AI to impersonate humans. Be transparent about AI involvement.",
    ...
    if (ghostwritingHits.length > 0) {
      ...
      ctx.addFinding({
        ...
        evidenceIds,
        recommendation: "Provide clear options for users to decide on attribution rather than defaulting to hidden AI authorship.",
      })
    }
    ...
    if (kbRules.length > 0 && (attributionHits.length > 0 || impersonationHits.length > 0)) {
      ctx.addFinding({
        ...
        evidenceIds: ctx.getEvidenceByType("attribution_rule").slice(0, 3).map((e) => e.id),
      })
    }
  }
}
