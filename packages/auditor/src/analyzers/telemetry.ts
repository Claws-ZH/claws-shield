	...
	async analyze(ctx: AuditContext, target: TargetInfo): Promise<void> {
		...
		if (foundEndpoints.size > 0 && !hasOptOut) {
			...
			ctx.addFinding({
				...
				evidenceIds,
				recommendation:
					"Provide a clear opt-out mechanism for all telemetry collection (CLI flag, env var, or config setting).",
				references: ["https://opentelemetry.io/docs/", "GDPR Article 7"],
			})
		}

		if (foundSdks.size > 0) {
			const sdkEvidence = ctx.evidence
				.filter((e) => e.metadata?.pattern === "sdk_import")
				.map((e) => e.id)
			...
			ctx.addFinding({
				...
				recommendation:
					"Document which third-party services receive telemetry data and what data is shared.",
			})
		}
		...
		if (fingerprintMatches.length > 5) {
			...
			ctx.addFinding({
				...
				recommendation:
					"Minimize collection of device-identifying information. Anonymize or hash hardware identifiers.",
			})
		}
		...
		if (retryMatches.length > 0 && foundEndpoints.size > 0) {
			ctx.addFinding({
				...
				description:
					"Retry patterns found alongside telemetry endpoints, indicating persistent telemetry delivery even on failure.",
				evidenceIds: ctx
					.getEvidenceByType("endpoint")
					.slice(0, 3)
					.map((e) => e.id),
