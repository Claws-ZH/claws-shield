...
export function formatMarkdown(report: AuditReport): string {
	...
	lines.push("")
	lines.push(
		`*Engine: v${report.metadata.engineVersion} | KB: v${report.metadata.kbVersion} | Analyzers: ${report.metadata.analyzersRun.join(", ")} | Duration: ${report.metadata.duration}ms*`,
	)
	lines.push("")
