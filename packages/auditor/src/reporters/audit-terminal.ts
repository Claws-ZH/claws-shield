...
import { gradeColor, RESET } from "@claws-shield/core"
...
export function formatTerminal(report: AuditReport): string {
  ...
  lines.push(`${BOLD}${gc}  ║  AUDIT GRADE:  ${report.overallGrade}  (${report.overallScore}/100)     ║${RESET}`)
  lines.push(`${BOLD}${gc}  ╚═══════════════════════════════╝${RESET}`)
  lines.push("")
  ...
  for (const [cat, label] of Object.entries(CATEGORY_LABELS) as [FindingCategory, string][]) {
    ...
    lines.push(`  ${label.padEnd(20)} ${catGc}${cs.grade.padEnd(8)}${RESET} ${String(cs.score).padEnd(10)} ${cs.issueCount}`)
  }
  lines.push(`${DIM}${"─".repeat(56)}${RESET}`)
  lines.push("")

  // Top findings (critical and high only for terminal)
  const topFindings = report.findings.filter((f) => f.severity === "critical" || f.severity === "high")
  if (topFindings.length > 0) {
    ...
    for (const finding of topFindings.slice(0, 10)) {
      ...
      lines.push(`  ${DIM}${finding.description.slice(0, 120)}${finding.description.length > 120 ? "..." : ""}${RESET}`)
      lines.push("")
    }
  }
  ...
  if (mediumCount + lowCount + infoCount > 0) {
    lines.push(`${DIM}  + ${mediumCount} medium, ${lowCount} low, ${infoCount} info finding(s)${RESET}`)
    lines.push("")
  }
  ...
  // Footer
  lines.push(`${DIM}Engine v${report.metadata.engineVersion} | KB v${report.metadata.kbVersion} | ${report.metadata.analyzersRun.length} analyzers | ${report.metadata.duration}ms${RESET}`)
  lines.push("")
