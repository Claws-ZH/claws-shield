...
import { RESET, findKBRoot, gradeColor, scoreToGrade } from "@claws-shield/core"
...
program
	...
	.action(
		async (
			target: string,
			opts: { format: string; out?: string; failBelow?: string; scoreOnly?: boolean },
		) => {
			try {
	...
	.action(async (skillPath: string, opts: { format: string; failOn?: string }) => {
		try {
			...
			if (opts.format === "json") {
				...
				console.log("\n  Skill Security Scan")
				console.log(
					`  Grade: ${color}${result.securityGrade}${RESET} (${result.securityScore}/100)`,
				)
				...
				if (result.findings.length > 0) {
					...
					for (const f of result.findings) {
						...
						console.log(
							`      ${f.location?.file ?? "unknown"}:${f.location?.line ?? "?"} — ${f.description}`,
						)
					}
				}
				console.log()
			...
			if (opts.codenames) {
				...
				if (opts.json) {
					...
					if (results.length === 0) {
						...
						for (const r of results) {
							console.log(
								`    ${r.codename} -> ${r.mapsTo || "unknown"} (${r.status || "unknown"})`,
							)
						}
						console.log()
					}
